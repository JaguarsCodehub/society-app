...
const Services = () => {
    ...
    return (
        <View>
            <ScrollView>
                <View>
                    <Text style={{ fontSize: 30, fontWeight: '700', marginLeft: 10 }}>Service Requests</Text>
                    {requestsData.map((item) => (
                        <View key={item.ID} style={{ backgroundColor: '#EAEAEA', padding: 10, margin: 10, borderRadius: 4 }}>
                            <Text style={{ fontSize: 20, fontWeight: '600', marginVertical: 1 }}>ServiceName: {item.ServiceName}</Text>
                            <Text style={{ fontSize: 20, fontWeight: '600', marginVertical: 1 }}>ServiceCode: {item.ServiceCode}</Text>
